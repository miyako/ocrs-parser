...
struct Args {
    ...
    /// Path to image to process. Defaults to stdin.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let image = match &args.input_path {
        Some(val) => image::open(val),
        None => {
            let mut buffer = Vec::new(); // read binary
            io::stdin().read_to_end(&mut buffer)?;
            if buffer.is_empty() {
                return Err("No `--input` provided and stdin was empty".into());
            }
    ...
    let color_img: NdTensor<u8, 3> = image
        .map(|image| {
        ...
        .with_context(|| format!("Failed to read image from --input or stdin"))?;
